...
//!
//! Extended for fork/wait: a `Proc` now records who its **parent** is and, once
//! it exits, the **exit status** it left behind. Those two fields are what let
//! `wait` (in syscall.rs) find a finished child and collect its result. Plus a
//! small `has_children` helper the scheduler and `wait` use. (UNDERSTAND —
//! given; the fork/wait/exit logic itself is in syscall.rs and usermode.rs.)
...
pub struct Proc {
    ...
    pub kstack: usize,
    /// The open-file table: fd `n` is `ofile[n]`.
    pub ofile: [File; NOFILE],
    /// Who forked this process (null for a process the kernel started
    /// directly). `wait` looks for children whose parent is itself.
    pub parent: *mut Proc,
    /// The status this process passed to `exit`, once it is a Zombie.
    pub xstate: isize,
    pub name: [u8; 16],
}
...
/// trampoline page and this process's own trapframe page. (UNDERSTAND — given.)
...
/// Does process `parent` have any child that still exists (any state but
/// Unused)? `wait` uses this to tell "no children at all" (return -1) apart
/// from "children exist but none has exited yet" (block and retry).
/// (UNDERSTAND — given.)
pub unsafe fn has_children(parent: *mut Proc) -> bool {
    for i in 0..NPROC {
        let q = proc_at(i);
        if (*q).state != ProcState::Unused && (*q).parent == parent {
            return true;
        }
    }
    false
