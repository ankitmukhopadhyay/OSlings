//! shell.rs — a tiny interactive shell with file commands, and (new in
//! exercise 19) `run PROGRAM [args...]` / `progs`, which launch programs
//! from the exec program table with command-line arguments.
...
impl Shell {
    ...
    /// Parse one input line and run the command it names. (Given — extended
    /// from exercise 17 with `run`.)
    pub fn exec(&mut self, line: &str, out: &mut dyn Out) {
        ...
        match cmd {
            ...
            "run" => self.cmd_run(line, out),
            "progs" => self.cmd_progs(out),
            _ => {
                out.puts(cmd);
    ...
    /// `touch NAME` — create an empty file. If it already exists, do nothing
    /// (that matches real `touch`).
    ...
    /// `echo TEXT > FILE` — write TEXT (plus a newline) into FILE, creating it
    /// if needed. With no `>`, just print TEXT.
    ...
    /// `run PROGRAM [args...]` — load a program from the exec table with its
    /// arguments, run it, and report how it ended. This is the shell doing
    /// what shells are FOR: starting a program with the arguments you typed,
    /// losing the CPU to it, and getting control back when it exits.
    /// (Given — new in exercise 19. Try `run echo hello world` or `run args`.)
    fn cmd_run(&mut self, line: &str, out: &mut dyn Out) {
        // The command line is "run PROGRAM arg1 arg2 ...".
        let mut words = line.split_whitespace();
        words.next(); // drop "run"
        let name = match words.next() {
            Some(n) => n,
            None => {
                out.puts("run: usage: run PROGRAM [args...]\n");
                return;
            }
        };
        ...
        unsafe {
            let p = match exec::exec(name, &args) {
                ...
                Err(exec::ExecError::NotFound) => {
                    out.puts("run: no such program: ");
                    out.puts(name);
                    out.puts("  (try `progs`)\n");
                    return;
                }
                Err(_) => {
                    out.puts("run: could not start the program\n");
                    return;
                }
            };
            let pid = (*p).pid;
            let outcome = usermode::run(p);
            crate::proc::freeproc(p);
            match outcome {
                RunOutcome::Exited(status) => {
                    out.puts("run: pid ");
                    put_num(out, pid as isize);
                    out.puts(" (");
                    out.puts(name);
                    out.puts(") exited with status ");
                    put_num(out, status);
                    out.puts("\n");
                }
                RunOutcome::Faulted(_) => out.puts("run: the program faulted\n"),
                RunOutcome::TimedOut => out.puts("run: gave up waiting\n"),
            }
        }
    }

    /// `progs` — list the programs `run` can launch. (Given.)
    fn cmd_progs(&self, out: &mut dyn Out) {
...
/// Print a signed number through an `Out` (no allocation, digits built
/// back-to-front in a small buffer). (Given.)
fn put_num(out: &mut dyn Out, n: isize) {
    if n < 0 {
        out.puts("-");
    }
