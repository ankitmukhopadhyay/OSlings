...
unsafe fn build_process(p: *mut Proc, name: &str, args: &[&str]) -> Result<(), ExecError> {
    ...
    //     Then `Ok(())`.
    // (USER_CODE and push_argv below are the given pieces you will use.)
    let _ = (p, name, args, USER_CODE, push_argv); // remove this line once implemented
    Err(ExecError::NoMem) // a program that was never built cannot run
}
