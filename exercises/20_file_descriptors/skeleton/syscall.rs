...
//!
//! Exercise 18 built the syscall machinery (a7 = number, a0..a2 = arguments,
//! a0 = return value) and three calls: exit, getpid, write. This exercise
//! adds the **file** calls — open, read, close — and turns `write` from a
//! console-only shim into a real fd operation. Now a user program can open a
//! file, read it a chunk at a time, write to it, and close it, all through
//! small integer **file descriptors** (see file.rs).
//!
...
/// Unknown numbers return -1. (Given — you wrote this match in exercise 18;
/// it has just grown three more arms.)
...
/// exit(status): the program is done. (UNDERSTAND — given.)
...
fn fdalloc(file: File) -> isize {
    // IMPLEMENT (the guidance is in the doc comment above).
    let _ = file; // remove once implemented
    -1
}
...
fn sys_open(path: usize, flags: usize) -> isize {
    // IMPLEMENT (the five steps are in the doc comment above). The items below
    // are the pieces you will use; delete these `let _` lines as you go.
    let _ = (path, flags, O_CREATE, O_WRONLY, O_RDWR, O_TRUNC, ROOT);
    let _ = InodeKind::File; // the kind to create for a new file
    let _ = &FS; // the filesystem lock
    let _ = fdalloc; // hand your finished File to this at the very end
    -1
}
...
fn sys_read(fd: usize, buf: usize, len: usize) -> isize {
    // IMPLEMENT (the guidance is in the doc comment above; read the given
    // `sys_write` below as your model — same shape, opposite direction).
    let _ = (fd, buf, len); // remove once implemented
    -1
}
