...
//! starts programs (the first half, `fork`, arrives in exercise 21).
//!
...
unsafe fn build_process(p: *mut Proc, name: &str, args: &[&str]) -> Result<(), ExecError> {
    ...
    (*tf).a0 = argc as u64;
    (*tf).a1 = argv as u64;
    // Make the process schedulable: the scheduler's first swtch into it lands
    // at forkret, which dives into user mode. (New in exercise 21.)
