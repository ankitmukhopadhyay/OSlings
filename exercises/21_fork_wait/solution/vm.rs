...
/// Copy a NUL-terminated STRING out of a user address space into `dst`,
/// stopping at the first zero byte. Returns the string's length (not counting
/// the NUL). `open` uses this to fetch the path a user program passed it — a
/// path is a string, and we don't know its length in advance, so we copy byte
/// by byte and stop at the terminator. (UNDERSTAND — given; it is `copyin`
/// with an early exit on the NUL.)
pub unsafe fn copyinstr(table: *mut Pte, dst: &mut [u8], mut srcva: usize) -> Result<usize, ()> {
    let mut got = 0;
    while got < dst.len() {
        let va0 = pgrounddown(srcva);
        let pa0 = walkaddr(table, va0);
        ...
        let off = srcva - va0;
        let n = PGSIZE - off; // bytes left on this page
        let src = (pa0 + off) as *const u8;
        for k in 0..n {
            if got == dst.len() {
                return Err(()); // filled the buffer without finding a NUL
            }
            let b = *src.add(k);
            dst[got] = b;
            if b == 0 {
                return Ok(got); // reached the terminator: done
            }
            got += 1;
        }
        srcva = va0 + PGSIZE; // NUL not on this page; continue on the next
    }
    Err(()) // no NUL within dst.len() bytes
}
...
/// Copy a parent's user address space into a child's (empty) page table: for
/// every user page mapped in `old`, allocate a fresh physical page in the
/// child, copy the parent's bytes into it, and map it at the same virtual
/// address with the same permissions. This is what makes `fork` give the child
/// its own private *copy* of the parent's memory — change one and the other is
/// untouched. (UNDERSTAND — given; it walks the page-table tree just like
/// `free_pt` above, but copying instead of freeing.)
pub unsafe fn uvmcopy(old: *mut Pte, new: *mut Pte) -> Result<(), ()> {
    copy_level(old, new, 2, 0)
}

/// Walk one level of `old`'s page table, reconstructing each entry's virtual
/// address as we descend, and copy every user leaf page into `new`.
unsafe fn copy_level(old: *mut Pte, new: *mut Pte, level: usize, base_va: usize) -> Result<(), ()> {
    for i in 0..512 {
        let pte = *old.add(i);
        if !pte.is_valid() {
            continue;
        }
        // this entry covers the virtual-address range starting here:
        let va = base_va | (i << (12 + level * 9));
        let flags = pte.flags();
        let is_leaf = flags & (PTE_R | PTE_W | PTE_X) != 0;
        if is_leaf {
            // only copy the user's own pages; the trampoline and trapframe
            // (no PTE_U) are installed separately by proc_pagetable.
            if flags & PTE_U != 0 {
                let dst = kalloc::kalloc();
                if dst.is_null() {
                    return Err(());
                }
                ptr::copy_nonoverlapping(pte.pa() as *const u8, dst, PGSIZE);
                if mappages(new, va, PGSIZE, dst as usize, flags).is_err() {
                    kalloc::kfree(dst);
                    return Err(());
                }
            }
        } else {
            // an interior node: descend one level, extending the VA prefix.
            copy_level(pte.pa() as *mut Pte, new, level - 1, va)?;
        }
    }
    Ok(())
}
