...
///         (*child).pid as isize
fn sys_fork() -> isize {
    // IMPLEMENT (the six steps are in the doc comment above). `ProcState`
    // and the `proc`/`vm`/`usermode` helpers named there are what you need.
    let _ = ProcState::Runnable; // remove once implemented
    -1
}
...
fn sys_wait(status_addr: usize) -> isize {
    unsafe {
        ...
        loop {
            // IMPLEMENT: scan for a ZOMBIE child of `p` and reap it (see the doc
            // comment above). Walk `0..NPROC` with `proc::proc_at(i)`.
            let _ = (NPROC, status_addr); // remove once implemented
