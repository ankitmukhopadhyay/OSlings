...
pub unsafe fn map_user_pages(
    ...
    let _ = (table, code_page, stack_page); // remove once implemented
    Ok(())
}
...
pub unsafe fn copyin(table: *mut Pte, dst: &mut [u8], srcva: usize) -> Result<(), ()> {
    ...
    // `dst.as_mut_ptr().add(i)` points i bytes into `dst`. You will advance
    // `srcva` from page to page, so first make it mutable by writing the
    // parameter as `mut srcva: usize` — compare copyout's signature above.)
    let _ = (table, dst, srcva); // remove once implemented
    Err(()) // an unimplemented copyin must not pretend it copied anything
}
