...
//! exercise 18) a `run` command that launches the embedded user program.
...
impl Shell {
    ...
    /// `run` — launch the embedded user program, wait for it, and report how
    /// it ended. This is the shell doing what shells are FOR: starting a
    /// program, losing the CPU to it, and getting control back when it exits.
    /// (Given — new in exercise 18. Run it twice and watch the pid climb.)
    fn cmd_run(&mut self, out: &mut dyn Out) {
        unsafe {
            let p = usermode::setup();
            if p.is_null() {
                out.puts("run: could not create the process\n");
                return;
            }
            let pid = (*p).pid;
