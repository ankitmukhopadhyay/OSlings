...
pub extern "C" fn kerneltrap() {
    unsafe {
        ...
        // exception (0).
        if (scause >> 63) == 1 {
            // an interrupt. Look at the cause code (the low bits).
            match scause & 0xff {
                1 => {
                    // supervisor software interrupt = our forwarded timer tick.
                ...
                9 => {
                    // supervisor external interrupt = a device (the UART) via
                    // the PLIC. Let the console handle it.
                    crate::console::intr();
                }
                _ => {}
            }
